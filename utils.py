...
#def onObjetClick(event):
    #print('Got objetbt click', event.x, event.y)
    #print(event.widget.find_closte(event.x,event.y))
...
def dessiner_jeton(self, jeton, i, j, nb_pixels_per_case, tag='lettre'):
    ...
    rectangle = self.create_rectangle(debut_colonne, debut_ligne, fin_colonne, fin_ligne, fill='#b9936c', tags=tag)

    jeton = self.create_text((debut_colonne + d, debut_ligne + d),
                     font=('Times', '{}'.format(31)), text=str(jeton), tags='lettre')

    #self.tag_bind(tag, '<ButtonPress-1>', )
    #self.tag_bind(lettre, '<ButtonPress-1>')
